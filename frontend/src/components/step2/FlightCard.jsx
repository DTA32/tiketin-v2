    ...
    return(
        <div className="border border-secondary-subtle my-1 pt-2 pb-3 px-3 bg-white">
            ...
            <div className="container">
                ...
                <div className="col">
                    <div className="row">
                    <p className="col text-center mb-0">{format(parseISO(flightClass.penerbangan.waktu_berangkat), 'E, d LLL')}</p>
                    </div>
                    <div className="row text-center">
                    <p className="col mb-0">{format(parseISO(flightClass.penerbangan.waktu_berangkat), 'HH:mm')}</p>
                    <p
                        className="col mb-0 d-flex justify-content-center align-items-end"
                        style={{ fontSize: 12 }}
                    >
                        {intervalToDuration({start: parseISO(flightClass.penerbangan.waktu_berangkat),end: parseISO(flightClass.penerbangan.waktu_sampai),}).hours}j{" "}
                        {intervalToDuration({start: parseISO(flightClass.penerbangan.waktu_berangkat),end: parseISO(flightClass.penerbangan.waktu_sampai),}).minutes}m
                    </p>
                    <p className="fs-6 col mb-0">{format(parseISO(flightClass.penerbangan.waktu_sampai), 'HH:mm')}</p>
                    </div>
                    <div className="row text-center">
